...
func newTestBackend(t *testing.T) eventstore.Store {
	backend, e := NewBadgerBackend(t.TempDir())
	if e != nil {
...
func TestCombinedQueries(t *testing.T) {
	...
	e1 := makeEvent(1, k1, 1000, nostr.Tags{{"t", "bitcoin"}}, "e1") // pk1, k1, bitcoin
	e2 := makeEvent(1, k1, 2000, nostr.Tags{{"t", "nostr"}}, "e2")   // pk1, k1, nostr
	e3 := makeEvent(1, k2, 3000, nostr.Tags{{"t", "bitcoin"}}, "e3") // pk2, k1, bitcoin
...
func TestComplexTagQueriesCount(t *testing.T) {
	b := newTestBackend(t)
	defer b.Close()
