...
func (b *BoltBackend) countEventsHLLCached(filter nostr.Filter) (uint32, *hyperloglog.HyperLogLog, error) {
	...
	case 3:
		_ = xhex.Decode(cacheKey[2:2+8], []byte(filter.Tags["p"][0][0:8*2]))
	case 7:
		_ = xhex.Decode(cacheKey[2:2+8], []byte(filter.Tags["e"][0][0:8*2]))
	case 1111:
		_ = xhex.Decode(cacheKey[2:2+8], []byte(filter.Tags["E"][0][0:8*2]))
	}
...
func (b *BoltBackend) updateHyperLogLogCachedValues(txn *bbolt.Tx, evt nostr.Event) error {
	...
	for ref, offset := range nip45.HyperLogLogTargetsForEvent(evt) {
		...
		_ = xhex.Decode(cacheKey[2:2+8], []byte(ref[0:8*2]))
